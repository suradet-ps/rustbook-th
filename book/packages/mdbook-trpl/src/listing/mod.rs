...
/// ```
///
/// <figcaption>ลิสติ้ง 1-2: Some <em>text</em>, yeah?</figcaption>
///
/// </figure>
/// ````
///
    ...
    fn opening_html(&self) -> String {
        ...
        match self.file_name.as_ref() {
            Some(file_name) => format!(
                "{figure}<span class=\"file-name\">ชื่อไฟล์: {file_name}</span>\n",
            ),
            None => figure,
        }
    }
        ...
        match (&self.number, &self.caption) {
            (Some(number), caption) => {
                ...
                let listing_a_tag = format!(
                    "<a href=\"#listing-{number}\">ลิสติ้ง {number}</a>"
                );
                format!(
    ...
    fn closing_text(&self, trailing: &str) -> String {
        match (&self.number, &self.caption) {
            (Some(number), Some(caption)) => {
                format!("ลิสติ้ง {number}: {caption}{trailing}")
            }
            (None, Some(caption)) => format!("{caption}{trailing}"),
            (Some(number), None) => format!("ลิสติ้ง {number}{trailing}"),
